...
void COperatorDlg::Update_TrayInfo(int nTray, int nNo)
{
	...
	if (nTray == 0 || nTray == 2) {		// Angle Tray
		...
		for (int i = 0; i < gData.nTrayY; i++) {
			for (int j = 0; j < gData.nTrayX; j++) {
				if		(gData.InfoAngleTray[nNo][i][j] == 9) m_grdOperAngleTray.Set_CellBackClr(i, j, RGB(0x00, 0xFF, 0xFF));	// Reserve
				else if (gData.InfoAngleTray[nNo][i][j] == 0) m_grdOperAngleTray.Set_CellBackClr(i, j, RGB(0xFF, 0xFF, 0xFF));	// Empty
	...
	if (nTray == 0 || nTray == 3) {		// Buffer Tray
		...
		for (int i = 0; i < gData.nTrayY; i++) {
			for (int j = 0; j < gData.nTrayX; j++) {
				if		(gData.InfoBuffTray[nNo][i][j] == 1) m_grdOperBufferTray.Set_CellBackClr(i, j, RGB(0x00, 0xFF, 0x00));	// Good
				else if (gData.InfoBuffTray[nNo][i][j] == 0) m_grdOperBufferTray.Set_CellBackClr(i, j, RGB(0xFF, 0xFF, 0xFF));	// Empty
	...
	if (nTray == 0 || nTray == 5) {		// NG Tray
		for (int i = 0; i < gData.nTrayY; i++) {
			for (int j = 0; j < gData.nTrayX; j++) {
				if		(gData.InfoNgTray[nNo][i][j] == 2)	m_grdOperNgTray.Set_CellBackClr(i, j, RGB(0xFF, 0x00, 0x00));	// Normal
				else if	(gData.InfoNgTray[nNo][i][j] == 4)  m_grdOperNgTray.Set_CellBackClr(i, j, RGB(0xF0, 0xF0, 0x00));	// SN
				else if (gData.InfoNgTray[nNo][i][j] == 5)  m_grdOperNgTray.Set_CellBackClr(i, j, RGB(0xFF, 0x60, 0xFF));	// S2N
				else if (gData.InfoNgTray[nNo][i][j] == 0)  m_grdOperNgTray.Set_CellBackClr(i, j, RGB(0xFF, 0xFF, 0xFF));	// Empty
